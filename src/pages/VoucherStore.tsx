...
import {
  ...
  ChevronDown, ChevronUp, Flag, KeyRound
} from 'lucide-react';
import {
...
function OrderHistorySection({ vouchers }: { vouchers: PurchasedVoucher[] }) {
  const sorted = [...vouchers].sort((a, b) => new Date(b.purchased_at).getTime() - new Date(a.purchased_at).getTime());

  return (
    <div>
      <div className="border-t border-border/40 pt-8 mt-10 mb-6">
        <h2 className="text-xl font-bold text-foreground font-display">Order History</h2>
        <p className="text-sm text-muted-foreground mt-1">Complete record of all your voucher purchases</p>
      </div>
        ...
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border/40 bg-muted/30">
                  <th className="text-left px-4 py-3 text-xs text-muted-foreground font-medium uppercase tracking-wider">Order ID</th>
                  <th className="text-left px-4 py-3 text-xs text-muted-foreground font-medium uppercase tracking-wider">Voucher</th>
                  <th className="text-left px-4 py-3 text-xs text-muted-foreground font-medium uppercase tracking-wider">Platform</th>
                  <th className="text-left px-4 py-3 text-xs text-muted-foreground font-medium uppercase tracking-wider">Amount</th>
              ...
              <tbody>
                {sorted.map(pv => {
                  const meta = getVoucherMeta(pv.voucher_id);
                  return (
                    <tr key={pv.id} className="border-b border-border/20 last:border-0 hover:bg-muted/10 transition-colors">
                      <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{pv.id.slice(0, 8).toUpperCase()}</td>
                      <td className="px-4 py-3 font-medium text-foreground">{meta?.brand || 'Voucher'} {pv.variant_label && `(${pv.variant_label})`}</td>
                      <td className="px-4 py-3 text-muted-foreground">{pv.platform}</td>
                      <td className="px-4 py-3 text-muted-foreground">{pv.variant_label || '—'}</td>
                      <td className="px-4 py-3 text-muted-foreground">{formatDate(pv.purchased_at)}</td>
                      <td className="px-4 py-3">
                        <Badge className={`text-xs ${
                          pv.status === 'used' ? 'bg-muted text-muted-foreground' :
                          pv.status === 'expired' ? 'bg-destructive/20 text-destructive' :
                          'bg-green-500/20 text-green-400'
                        }`}>
                          {pv.status.charAt(0).toUpperCase() + pv.status.slice(1)}
                        </Badge>
          ...
          <div className="md:hidden space-y-3">
            {sorted.map(pv => {
              const meta = getVoucherMeta(pv.voucher_id);
              return (
                <div key={pv.id} className="rounded-xl border border-border/50 bg-card/50 p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs text-muted-foreground">{pv.id.slice(0, 8).toUpperCase()}</span>
                    <Badge className={`text-xs ${
                      pv.status === 'used' ? 'bg-muted text-muted-foreground' :
                      pv.status === 'expired' ? 'bg-destructive/20 text-destructive' :
                      'bg-green-500/20 text-green-400'
                  ...
                  <p className="font-medium text-foreground text-sm">{meta?.brand || 'Voucher'} {pv.variant_label && `(${pv.variant_label})`}</p>
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>{pv.platform}</span>
                    <span>{formatDate(pv.purchased_at)}</span>
                  </div>
                </div>
              );
            })}
